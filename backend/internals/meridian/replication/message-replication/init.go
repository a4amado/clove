...
var messageReplicationInstance *MessageReplication
...
func ReplicateMessage() *MessageReplication {
	...
	messageReplicationOnce.Do(func() {
		...
		// Declare topic exchange for message replication
		exchangeName := "message-replication-exchange"
		...
		localQueueName := fmt.Sprintf("%s-msg-replication-queue", localRegion)
		...
		routingKey := fmt.Sprintf("%s.msg-replication", localRegion)
		...
		messageReplicationInstance = &MessageReplication{
			conn:                 valkeyPool.Client(valkeyPool.ValkeyFanout),
	...
	return messageReplicationInstance
}
