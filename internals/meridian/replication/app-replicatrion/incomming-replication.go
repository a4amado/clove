...
func (c *AppReplication) BridgeKafkaAppReplicatorToRedis(ctx context.Context) {
	if c.replicatableAppMsgReader == nil {
	...
	for {
		...
		case <-ctx.Done():
			return
		default:
			kafkaMessage, err := c.replicatableAppMsgReader.ReadMessage(ctx)
			if err != nil {
			...
			err = c.replicatableAppMsgReader.CommitMessages(ctx, kafkaMessage)
			if err != nil {
